"""
VCPBacktester v3 — realistic exit management based on Minervini's actual rules.

Key changes from v2:
- Breakeven move at 2R (not 1R) — gives trade room to breathe after initial thrust
- No mechanical trailing stop until 2R — use hard structural stop only in early phase
- After 2R: trail at 85% (15% below current) — wide enough for volatile momentum stocks
- After 3R: tighten trail to 90% (10% below)
- Time stop: 20 days at <0.2R — gives setups time to develop
- 21-day EMA stop as alternative trail (whichever is higher)
"""
...
class VCPBacktester:
    ...
    def _sell(self, trade, qty, price):
        ...
        self.cash += proceeds
        trade.remaining_shares -= qty

    def _close(self, trade, date_str, price, reason):
        if trade.remaining_shares > 0: self._sell(trade, trade.remaining_shares, price)
        trade.exit_date=date_str; trade.exit_price=price
        trade.exit_reason=reason; trade.status="CLOSED"
    ...
    def _manage(self, trade, row, date_str):
        ...
        # ── Phase 1: Days 1–5 — hard stop only, no trail ────────────
        # Give the breakout room to establish immediately without
        # getting shaken out by normal post-breakout volatility
        if trade.days_in_trade <= 5:
            eff_stop = trade.stop_price
            if low <= eff_stop:
                self._close(trade, date_str, eff_stop * (1 - self.slippage), "STOP")
                return
            return

        # ── Phase 2: Beyond day 5 — trail activates ──────────────────
        # Trail width depends on how much profit we're sitting on
        if r >= 3.0:
            trail_pct = 0.90   # 10% below — tighter once well ahead
        elif r >= 2.0:
            trail_pct = 0.85   # 15% below
        else:
            trail_pct = 0.82   # 18% below — loose while position is developing

        if trade.trail_stop is None:
            trade.trail_stop = trade.stop_price
        trade.trail_stop = max(trade.trail_stop, price * trail_pct)

        # ── Move stop to breakeven at 2R (not 1R) ───────────────────
        if not trade.at_breakeven and r >= 2.0:
        ...
        if trade.remaining_shares <= 0:
            trade.exit_date=date_str; trade.exit_price=price
            trade.exit_reason="FULL_PARTIAL"; trade.status="CLOSED"
            self.closed_trades.append(trade)
            if trade in self.open_trades: self.open_trades.remove(trade)
            return
        ...
        # ── Time stop: 20 days with <0.2R gain ──────────────────────
        # Only if stock has never reached 1R — if it got there and pulled
        # back, that's a different situation, let it run
        if trade.days_in_trade >= 20 and r < 0.2 and trade.current_r(trade.high_water) < 1.0:
            self._close(trade, date_str, price * (1 - self.slippage), "TIME_STOP")
            return

        # ── Climax top: large single-day surge after big run ─────────
        if r >= 5.0:
            open_price = float(row["Open"])
            if open_price > 0 and (price - open_price) / open_price > 0.07:
    ...
    def _check_orders(self, date_str, day_num):
        ...
        for ticker, order in self.pending_orders.items():
            ...
            self.open_trades.append(Trade(
                ticker=ticker, entry_date=date_str, entry_price=entry,
                stop_price=stop, pivot=order["pivot"], shares=shares,
                initial_risk=dollar_risk, quality=order["quality"], rs_rating=order["rs"]
            ))
    ...
    def run(self):
            ...
            if day_num >= self.warmup and day_num % 5 == 0 and regime_ok:
                ...
                for ticker, df in self.stocks.items():
                    ...
                    self.pending_orders[ticker] = {
                        "pivot": sig["pivot"], "stop": sig["stop"],
                        "buy_stop": buy_stop, "quality": sig["quality"],
                        "rs": rs, "expires": expires,
                    }
                    self.scan_log.append({
                        "date": date_str, "ticker": ticker,
                        "pivot": sig["pivot"], "quality": sig["quality"], "rs": rs,
                    })
        ...
        for trade in list(self.open_trades):
            df = self.stocks.get(trade.ticker)
            if df is not None:
                self._close(trade, last_date, float(df["Close"].iloc[-1]), "END_OF_BACKTEST")

        eq = pd.Series([v for _, v in self.equity_history],
                       index=pd.to_datetime([d for d, _ in self.equity_history]))
        hs = pd.Series([v for _, v in self.heat_history],
                       index=pd.to_datetime([d for d, _ in self.heat_history]))
        rs = pd.Series([v for _, v in self.regime_history],
                       index=pd.to_datetime([d for d, _ in self.regime_history]))
        return BacktestResult(
            trades=self.closed_trades + self.open_trades,
            equity_curve=eq, daily_heat=hs, regime_series=rs, scan_log=self.scan_log
        )
